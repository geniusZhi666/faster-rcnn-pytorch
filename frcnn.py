...
class FRCNN(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"    : 'logs\ep010-loss0.954-val_loss0.886.pth',
 ...
 #---------------------------------------------------#
    #   检测图片，只显示rpn网络的预测结果
    #---------------------------------------------------#
    def detect_rpn_image(self, image, crop = False, count = False):
        #---------------------------------------------------#
        #   计算输入图片的高和宽
        #---------------------------------------------------#
        image_shape = np.array(np.shape(image)[0:2])
        ...
        with torch.no_grad():
            ...
            #   获得建议框
            #---------------------------------#
            _, _, rois, _, _ = self.net(images,mode="rpn")
        #-------------------------------------------------------------#
        # 根据 input_shape, image_shape 对 rois 进行调整
        #-------------------------------------------------------------#
        
        
        #将rois转换成numpy
        rois = rois.cpu().numpy()[0]
        #-------------------------------------------------------------#
        #   对预测框进行归一化，调整到0-1之间
        #-------------------------------------------------------------#
        ...
        boxs= self.bbox_util.frcnn_correct_boxes(box_xy, box_wh, input_shape, image_shape)

        #top_scores = rpn_scores[0].cpu().numpy()
        #top_boxes   = rois[0][:, :4]
        #-------------------------------------------------------------#
        #  在图像中绘制候选框 
        ...
        #   图像绘制
        #---------------------------------------------------------#
        #print(rois.shape) torch.Size([1, 300, 4])
        #print(boxs.shape)
        thickness   = int(max((image.size[0] + image.size[1]) // np.mean(input_shape), 1))
        for i in range(20): #设置绘制的候选框数量
            box = boxs[i]
            top, left, bottom, right = box
            top     = max(0, np.floor(top).astype('int32'))
            left    = max(0, np.floor(left).astype('int32'))
